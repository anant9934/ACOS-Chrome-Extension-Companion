...
function injectOptimizationUI(textarea: HTMLElement, adapter: any) {
  ...
  feedbackContainer.style.cssText = "display: none; background: #1e293b; border: 1px border #334155; padding: 4px 8px; border-radius: 6px; gap: 8px; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1);"
  ...
  btn.onclick = async (e) => {
    ...
    const originalTokens = Math.ceil(originalText.length / 4) // Simplified for instant feedback

    chrome.runtime.sendMessage({
      type: "ANALYZE_RELEVANCE",
      input: { prompt: originalText }
    }, (response) => {
      const optimizedTokens = Math.ceil(originalText.length / 4 * 0.7) // Mocked reduction
      
      // Update Textarea
      const optimizedText = `[ACOS OPTIMIZED] ${originalText}` // In reality, use the actual optimized text
  ...
  feedbackContainer.querySelector("#acos-down")?.addEventListener("click", () => {
    feedbackContainer.innerHTML = '<span style="color: #f43f5e; font-size: 11px; font-weight: bold;">Noted. Improving...</span>'
    setTimeout(() => feedbackContainer.style.display = "none", 2000)
  })
}
